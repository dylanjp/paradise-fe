...
export default function NotificationForm({
  ...
  useEffect(() => {
    if (isAdmin && currentUserId && users.length > 0 && !formState.isGlobal) {
      ...
      if (
        currentUserInList &&
        !formState.selectedUserIds.includes(currentUserInList.id)
      ) {
        // Replace username with actual user ID if needed
        setFormState((prev) => ({
          ...prev,
          selectedUserIds: prev.selectedUserIds
            .map((id) => (id === currentUserId ? currentUserInList.id : id))
  ...
  const validateForm = useCallback(() => {
    ...
    if (
      ...
      newErrors.selectedUserIds =
        "At least one user must be selected for non-global notifications";
    }
    ...
    if (formState.hasActionItem && !formState.actionDescription.trim()) {
      newErrors.actionDescription =
        "Action description is required when action item is enabled";
    } else if (formState.actionDescription.length > 500) {
      newErrors.actionDescription =
        "Action description must be 500 characters or less";
    }
  ...
  return (
    <form className={styles.form} onSubmit={handleSubmit}>
      ...
      <div className={styles.fieldGroup}>
        ...
        />
        {errors.messageBody && (
          <span className={styles.errorText}>{errors.messageBody}</span>
        )}
        <span className={styles.charCount}>
          {formState.messageBody.length}/2000
        </span>
      </div>
      ...
      {isAdmin && !formState.isGlobal && (
        <div className={styles.fieldGroup}>
          <label className={styles.label}>
            Assign to Users <span className={styles.required}>*</span>
          </label>
          {usersLoading ? (
            ...
            <div className={styles.userList}>
              ...
              <span className={styles.helpText}>
                User list unavailable - showing current user only
              </span>
            </div>
          ) : (
            <div className={styles.userList}>
          ...
          {errors.selectedUserIds && (
            <span className={styles.errorText}>{errors.selectedUserIds}</span>
          )}
        </div>
      )}
      ...
      {!isAdmin && (
        <div className={styles.fieldGroup}>
          <span className={styles.helpText}>
            This notification will be assigned to you.
          </span>
        </div>
      )}
      ...
      {formState.hasExpiration && (
        <div className={styles.fieldGroup}>
          ...
          />
          {errors.expiresAt && (
            <span className={styles.errorText}>{errors.expiresAt}</span>
          )}
        </div>
      )}
      ...
      <div className={styles.checkboxGroup}>
        <label className={styles.checkboxLabel}>
          <input
            type="checkbox"
            name="hasRecurrence"
            checked={formState.hasRecurrence}
            onChange={handleInputChange}
            className={styles.checkbox}
            disabled={isSubmitting}
          />
      ...
      {formState.hasRecurrence && (
        <div className={styles.recurrenceSection}>
          ...
          {formState.frequency === "YEARLY" && (
            <>
              <div className={styles.fieldGroup}>
                <label htmlFor="yearlyMonth" className={styles.label}>
                  Month
                </label>
                <select
                  id="yearlyMonth"
                ...
                {errors.yearlyMonth && (
                  <span className={styles.errorText}>{errors.yearlyMonth}</span>
                )}
              </div>

              <div className={styles.fieldGroup}>
                <label htmlFor="yearlyDay" className={styles.label}>
                  Day
                </label>
                <input
                  type="number"
                  ...
                  min={1}
                  max={
                    formState.yearlyMonth
                      ? getMaxDayForMonth(Number(formState.yearlyMonth))
                      : 31
                  }
                  placeholder="Day"
                  disabled={isSubmitting}
                />
                {errors.yearlyDay && (
                  <span className={styles.errorText}>{errors.yearlyDay}</span>
                )}
              </div>

              {formState.yearlyMonth === "2" &&
                formState.yearlyDay === "29" && (
          ...
          {formState.frequency === "RANDOM_DATE_RANGE" && (
            <>
                ...
                <div className={styles.dateRangeGroup}>
                  ...
                  <div className={styles.dateRangeInputs}>
                    ...
                    <input
                      ...
                      min={1}
                      max={
                        formState.rangeStartMonth
                          ? getMaxDayForMonth(Number(formState.rangeStartMonth))
                          : 31
                      }
                      placeholder="Day"
                      disabled={isSubmitting}
                  ...
                  {errors.rangeStartMonth && (
                    <span className={styles.errorText}>
                      {errors.rangeStartMonth}
                    </span>
                  )}
                  {errors.rangeStartDay && (
                    <span className={styles.errorText}>
                      {errors.rangeStartDay}
                    </span>
                  )}
                ...
                <div className={styles.dateRangeGroup}>
                  ...
                  <div className={styles.dateRangeInputs}>
                    ...
                    <input
                      ...
                      min={1}
                      max={
                        formState.rangeEndMonth
                          ? getMaxDayForMonth(Number(formState.rangeEndMonth))
                          : 31
                      }
                      placeholder="Day"
                      disabled={isSubmitting}
              ...
              {formState.rangeStartMonth &&
                formState.rangeStartDay &&
                formState.rangeEndMonth &&
                formState.rangeEndDay && (
                  <div className={styles.recurrencePreview}>
                    {formatRecurrencePreview({
      ...
      <div className={styles.checkboxGroup}>
        <label className={styles.checkboxLabel}>
          <input
            type="checkbox"
            name="hasActionItem"
            checked={formState.hasActionItem}
            onChange={handleInputChange}
            className={styles.checkbox}
            disabled={isSubmitting}
          />
        ...
        <span className={styles.helpText}>
          Allow users to create a TODO from this notification
        </span>
      </div>

      {formState.hasActionItem && (
        <div className={styles.fieldGroup}>
          <label htmlFor="actionDescription" className={styles.label}>
            Action Description <span className={styles.required}>*</span>
          </label>
          <input
            type="text"
            ...
            onChange={handleInputChange}
            className={`${styles.input} ${errors.actionDescription ? styles.inputError : ""}`}
            placeholder="Describe the action to be taken"
            maxLength={500}
            disabled={isSubmitting}
          />
          {errors.actionDescription && (
            <span className={styles.errorText}>{errors.actionDescription}</span>
          )}
          <span className={styles.charCount}>
            {formState.actionDescription.length}/500
          </span>
        </div>
      )}
