...
const ERROR_MESSAGES = {
  ...
  DRIVE_UNAVAILABLE: "The drive service is temporarily unavailable. Please try again later.",
...
export async function createFolder(userId, driveKey, name, parentId) {
  return apiClient.post(`/users/${userId}/drives/${driveKey}/folders`, { name, parentId });
}

/** PUT /users/{userId}/drives/{driveKey}/items/{itemId} */
export async function updateItem(userId, driveKey, itemId, updates) {
  return apiClient.put(`/users/${userId}/drives/${driveKey}/items/${itemId}`, updates);
}
...
export async function downloadFile(userId, driveKey, itemId) {
  ...
  if (!response.ok) {
    ...
    throw new apiClient.ApiError(errorData?.message || "Download failed", response.status, errorData);
  }
  ...
  return new Promise((resolve, reject) => {
    ...
    xhr.onload = () => {
      if (xhr.status >= 200 && xhr.status < 300) {
        ...
        let errorData = null;
        try { errorData = JSON.parse(xhr.responseText); } catch { /* not JSON */ }
        reject(new apiClient.ApiError(errorData?.message || "Upload failed", xhr.status, errorData));
      }
    };

    xhr.onerror = () => reject(new apiClient.ApiError("Upload failed", 0, null));
    xhr.send(formData);
  });
}
  ...
  return new Promise((resolve, reject) => {
    ...
    xhr.onload = () => {
      if (xhr.status >= 200 && xhr.status < 300) {
        ...
        let errorData = null;
        try { errorData = JSON.parse(xhr.responseText); } catch { /* not JSON */ }
        reject(new apiClient.ApiError(errorData?.message || "Plex upload failed", xhr.status, errorData));
      }
    };

    xhr.onerror = () => reject(new apiClient.ApiError("Plex upload failed", 0, null));
    xhr.send(formData);
  });
}
